(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.qrcode1) {
      window.wwload.qrcode1 = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/jquery.qrcode/jquery.qrcode", "libs/jquery.qrcode/utf"], function () {
        window.wwload.qrcode1 = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.qrcode1 === "wait") {
      setTimeout(function () {
   ...
   */
  function jq_qrcode($ele) {
    var config = {};
    config.qrcodeid = $ele.attr("data-qrcodeid");
    // config.downloadlinkid = $ele.attr("data-downloadlinkid");
    config.width = eval($ele.attr("data-width") || "200");
    config.height = eval($ele.attr("data-height") || "200");
    config.background = $ele.attr("data-background") || "#ffffff";
    config.foreground = $ele.attr("data-foreground") || "#000000";
    config.text = $ele.attr("data--text") || "扫描二维码后显示的内容";  //https://www.baidu.com/
    config.src = $ele.attr("data-src") || "";
    config.imgWidth = eval($ele.attr("data-imgwidth") || "50");
    config.imgHeight = eval($ele.attr("data-imgheight") || "50");
    config.qrcodename = $ele.attr("data-qrcodename") || "二维码.png";
    config.qrcodetime = eval($ele.attr("data-qrcodetime") || "1000");

    $ele.find(".qrcodeCanvas").attr("id", config.qrcodeid);
    // $ele.find(".downloadLink").attr("id", config.downloadlinkid);
    $ele.find('#' + config.qrcodeid).qrcode({
      render: "canvas", //设置渲染方式，有table和canvas，使用canvas方式渲染性能相对来说比较好
      text: config.text, //扫描二维码后显示的内容,可以直接填一个网址，扫描二维码后自动跳向该链接
      width: config.width, //二维码的宽度
      height: config.height,
      background: config.background, //二维码的后景色
      foreground: config.foreground, //二维码的前景色
      src: config.src,
      imgWidth: config.imgWidth,
      imgHeight: config.imgHeight,
    });

    var canvas = $ele.find('#' + config.qrcodeid).find("canvas").get(0);
    //console.log(canvas);         
    setTimeout(function () {
      var url = canvas.toDataURL('image/png');
      //console.log(url);
      $.wwclass.helper.updateProp($ele, "data-x-qrcodeimg", url);    //输出base64格式图片
      //var imgFile = dataURLtoBlob(url);
      //console.log(imgFile);

      // $('#' + config.downloadlinkid).attr("href", url).attr("download", config.qrcodename); //添加下载路径
    }, config.qrcodetime)
    ...
    switch (attribute) {
      case "data--text":
        if (value) {
          ...
          config.qrcodeid = $ele.attr("data-qrcodeid");
          setTimeout(function () {
            $ele.find('#' + config.qrcodeid).find("canvas").get(0).remove();
            jq_qrcode($ele);
          }, 1000)
        }
        break;
